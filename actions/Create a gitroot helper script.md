...
- [ ] Must be able to execute on non-nixos machines that can have nix installed
- [ ] Must be able to execute for any repo in said machine
...
You could do the same on a nixos machine but then that config would not be transferable onto systems that only have access to nix, not nixos
